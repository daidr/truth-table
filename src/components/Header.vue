...
<template>
  <header class="header">
    ...
    <div class="toolbar">
      <a
        href="https://github.com/daidr/truth-table"
        target="_blank"
      ...
      <select v-model="locale">
        <option v-for="item of availableLocales" :value="item">
          {{ t(`locate.${item}`) }}
        </option>
      </select>
    </div>
...
.header {
  ...
  @apply px-4 md:px-20 py-4 mx-2 sm:mx-4 my-4;
}
...
.toolbar select {
  ...
  @apply text-base md:text-lg select-none cursor-pointer appearance-none text-center;
}
